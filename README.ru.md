...
Это API использует C++ 17, поддерживает Linux (g++) и Windows (Visual Studio 2019/2022).
...
* Клонируйте репозеторий библиотеки
```bash
git clone https://github.com/green-api/whatsapp-api-client-cpp.git
```  

* Откройте CMakeFile в Visual Studio.
Файлы->Открыть->CMake->Каталог whatsapp-api-client-cpp

#### Подключение готовой библиотеки
Так же в репозитории (каталог build) выложена уже собранная библиотека (.lib, .a), вы можете скачать и установить ее в зависимости вашего окружения.

Подключение библиотеки в Visual Studio можно сделать
добавив файл библиотеки .lib в зависимости проекта:
1. Свойства->Свойства конфигурации->Компоновщик->Ввод->Дополнительные зависимости, указать фаил библиотеки (.lib)
2. Свойства->Свойства конфигурации->С/С++->Общие->Дополнительные каталоги включаемых файлов, указать файлы заголовков (папка include)
...
Вы можете установить зависимости с помощью готового сценария **install.sh** (не забудьте предоставить ему разрешения администратора) или вручную:
```bash
git clone https://github.com/Microsoft/vcpkg.git
cd vcpkg
./bootstrap-vcpkg.sh
...
Сборка библиотеки осуществляется с помощию сценария **build.sh** или CMake:
```bash
mkdir build
...
### Пример отправки текстового сообщения на номер WhatsApp
...
Для запуска конкретного примера необходимо производить копирование кода примера в ваш рабочий фаил main.cpp и параметризирование метода вашими значениями.
